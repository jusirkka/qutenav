...
void ChartPainter::paintGL(const Camera *cam) {
  ...
  funcs->glViewport(0, 0,
                    cam->heightMM() * cam->aspect() * Platform::dots_per_mm_x(),
                    cam->heightMM() * Platform::dots_per_mm_y());
...
void ChartPainter::updateCharts(const Camera* cam, const QRectF& viewArea) {
  ...
  qreal scale = .5 * bufCam->heightMM() * Platform::dots_per_mm_y() * bufCam->projection()(1, 1);
