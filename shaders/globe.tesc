...
void main() {
  gl_TessLevelOuter[0] = tess_clamp(detail(normal(coord[1]), normal(coord[2])));
  gl_TessLevelOuter[1] = tess_clamp(detail(normal(coord[2]), normal(coord[0])));
  gl_TessLevelOuter[2] = tess_clamp(detail(normal(coord[0]), normal(coord[1])));
  gl_TessLevelInner[0] = min(gl_TessLevelOuter[0],
                              min(gl_TessLevelOuter[1],
                                  gl_TessLevelOuter[2]));
  corner[0] = coord[0];
  corner[1] = coord[1];
  corner[2] = coord[2];
